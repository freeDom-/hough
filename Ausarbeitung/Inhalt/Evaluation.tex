...
\begin{table}[!ht]
	...
	\resizebox{\textwidth}{!}{
		\begin{tabular}{lrrrr}
			...
			\acs{fps} & \textbf{30,67} & 7,81 & 0,9 & 0,23 \\
			\hline
			\acs{BRAM} & 40 (2,19\%) & 141 (7,73\%) & \textbf{545 (29,88\%)} & \textbf{1.091 (59,81\%)} \\
			\acs{DSP}48E & 19 (0,75\%) & 19 (0,75\%) & 32 (1,27\%) & 32 (1,27\%) \\
			\acs{FF} & 3.233 (0,59\%) & 3.911 (0,71\%) & 5.125 (0,93\%) & 9.815 (1,79\%) \\
			\acs{LUT} & 7.477 (2,73\%) & 8.764 (3,20\%) & 9.425 (3,44\%) & 12.054 (4,40\%) \\
...
Wie in \autoref{img:fps} zu sehen, erreicht die Implementierung der \ac{CHT} für die vier Testbilder nur beim kleinsten Bild ausreichend \ac{fps}, um in Echtzeit auf ein Videosignal angewandt werden zu können. Ab einer Bildgröße von $800 \cdot 800$ Pixel kann nicht mal mehr ein Bild pro Sekunde verarbeitet werden. Zur Erkennung von Objekten im Straßenverkehr eignet sich die \ac{HT} daher selbst mit Hardwarebeschleunigung nicht.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Bilder/fps.pdf}
	\caption[Leistungsvergleich bezüglich der Echtzeitfähigkeit der \acl{CHT}]{Leistungsvergleich bezüglich der Echtzeitfähigkeit der \ac{CHT}}
	\label{img:fps}
\end{figure}

Durch die vielen von der \ac{HT} benötigten Parameter eignet das Verfahren nur schlecht unter wechselnden Bedingungen, wie sich verändernden Lichtverhältnissen. Mit verschiedenen Kernelgrößen für das Gauß-Filter und verschiedenen Schwellwerten für die Canny Edge Detection werden unterschiedlich gute Ergebnisse erzielt. Die Parameter der \ac{HT} müssen vor der Synthese, z.B. durch a priori Wissen, bestimmt werden. Wenn diese einmal in der \ac{PL} implementiert sind, lassen sie sich erst über eine erneute Synthese wieder verändern. Dadurch ist das Design nicht so flexibel wie die Softwarelösung.
\\
...
\\
\\
In dem Beispielbild wird jedoch trotz der geringen Ressourcennutzung für alle Bildgrößen ein sehr guter Speedup gegenüber der reinen Softwareimplementierung erzielt. Der Zynq \linebreak Ultrascale+ MPSoC eignet sich gut, um Teile eines Algorithmus, hier insbesondere das kostenintensive Votingverfahren der \ac{CHT}, auszulagern und zu beschleunigen.
